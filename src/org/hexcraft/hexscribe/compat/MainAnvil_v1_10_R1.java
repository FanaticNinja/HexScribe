package org.hexcraft.hexscribe.compat;

import org.hexcraft.HexScribe;
import org.hexcraft.Anvil.AnvilView;
...
public class MainAnvil_v1_10_R1 implements AnvilView
{

    final Player player;
    final HexScribe plugin;
    CraftInventoryAnvil inv;
    ContainerAnvil anvil;
    int repairCost;
    ItemStack secondItem;

    public MainAnvil_v1_10_R1(HexScribe plugin, Inventory anvil, Player player)
    {
        this.player = player;
        this.plugin = plugin;
        ...
        final MainAnvil_v1_10_R1 main = this;
        plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable()
        {
            @Override
            public void run()
            {
                main.anvil = (ContainerAnvil)nmsPlayer.activeContainer;
            }
        }, 0);
    ...
    public String getNameText() {
        try {
            ...
            for (net.minecraft.server.v1_10_R1.ItemStack item : inv.getIngredientsInventory().getContents())
            {
            ...
            net.minecraft.server.v1_10_R1.ItemStack item = ((IInventory)g.get(anvil)).getItem(0);
            if (item == null)
                return null;
    ...
    @Override
    public void SetSecondItem(ItemStack item) {
    	secondItem = item;
    }
